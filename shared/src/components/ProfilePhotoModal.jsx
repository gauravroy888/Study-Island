...
import { X, User, Image as ImageIcon, Upload, ZoomIn, RotateCcw } from 'lucide-react';

import { supabase } from '../lib/supabase.js';

export default function ProfilePhotoModal({ isOpen, onClose, useTheme, profileName: propName, setProfileImage: propSetImage }) {
  const theme = typeof useTheme === 'function' ? useTheme() : {};
  const profileName = propName || theme.profileName || 'User';
  const setProfileImage = propSetImage || theme.setProfileImage || (() => {});
  ...
  // WCAG 2.2 Focus Trap & Keyboard Navigation (Escape & Tab trapping)
  useEffect(() => {
    if (!isOpen) return;
    ...
    // Initial focus on close button or first action
  ...
  const handleSaveToGlobal = async (imageUrl) => {
    ...
    try {
      ...
      if (userStr) {
        ...
        } catch { /* ignore teacher table update */ }
      }
    ...
    <div style={{
      ...
      <div 
        ...
        <div style={{
          ...
          <div>
            <h3 id="profile-modal-title" style={{ margin: 0, color: '#fff', fontSize: '1.25rem', fontWeight: '800' }}>
              Edit Profile Photo
            </h3>
